...
fn part1() -> Result<(), Box<dyn Error>>{
    let raw_input = read_to_string("/Users/jeffreybecca/projects/AoC/advent_of_rust/input/2023/day2.input")?;
    let given_cubes = CubeColors{red: 12, green: 13, blue: 14};
    ...
    for line in raw_input.lines() {
        ...
        let Some(game_id) = re_id.captures(id) else { panic!("did not find game id")};
        ...
        for pull in pulls.split(";") {
            let mut current_pull_values = CubeColors{red: 0, blue: 0, green: 0};
...
fn part2() -> Result<(), Box<dyn Error>> {
    ...
    for line in raw_input.lines() {
        let mut given_cubes = CubeColors{red: 0, green: 0, blue: 0};
        println!("line: {line}");
        let mut temp = line.split(":");
        let id = temp.next().unwrap();
        println!("id: {id}");
